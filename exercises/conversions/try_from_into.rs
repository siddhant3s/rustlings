...
impl<T> TryFrom<(T, T, T)> for Color where u8: TryFrom<T> {
...
impl<T: Copy> TryFrom<[T; 3]> for Color where u8: TryFrom<T>{
    type Error = IntoColorError;
...
impl<T>  TryFrom<&[T]> for Color where u8: TryFrom<T>, T: Copy {
    type Error = IntoColorError;
